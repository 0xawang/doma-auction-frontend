...
export function useAuctionsData(auctionCount: number) {
  const top15Ids = Array.from({ length: Math.min(15, auctionCount) }, (_, i) => auctionCount - i)
  ...
  const auctions: Auction[] = top15Ids.map((id, index) => {
    ...
    if (auctionResult?.status !== 'success' || !auctionResult.result) return null

    const data = auctionResult.result as unknown as any[]
    const tokenIds = tokensResult?.status === 'success' ? (tokensResult.result as unknown as bigint[]) : []
    const currentPrice = priceResult?.status === 'success' ? formatEther(priceResult.result as bigint) : '0'
    const currentRoyalty = royaltyResult?.status === 'success' ? Number(royaltyResult.result) / 100 : 0
    const filled = fillResult?.status === 'success' ? Number(fillResult.result as unknown as bigint) : 0
