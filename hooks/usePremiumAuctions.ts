...
export function usePremiumAuctions() {
  ...
  const auctions: (PremiumAuction | null)[] = auctionsData?.map((result, index) => {
    if (result.status === 'success' && result.result) {
      const data = result.result as unknown as readonly [string, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, boolean, boolean, string, bigint];
      const [seller, tokenId, startPrice, reservePrice, priceDecrement, startedAt, endedAt, highPrice, lowPrice, active, cleared, winner, finalPrice] = data;
  ...
  const {domainInfos} = useDomainMetadata(auctions?.map((auction) => auction?.tokenId.toString() || '[]') ) 
...
export function usePremiumAuction(id: string) {
  ...
  const {domainInfos} = useDomainMetadata(auction?.tokenId ? [auction?.tokenId?.toString()] : []) 
