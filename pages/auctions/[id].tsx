...
export default function AuctionDetailPage() {
  ...
  const {
    isOpen: isSoftBidOpen,
    onOpen: onSoftBidOpen,
    onClose: onSoftBidClose,
  } = useDisclosure();
  const {
    isOpen: isHardBidOpen,
    onOpen: onHardBidOpen,
    onClose: onHardBidClose,
  } = useDisclosure();
      ...
      <DefaultLayout>
        ...
        <div className="max-w-7xl mx-auto px-4 pb-8">
          <h1 className={title({ class: "gradient-metal" })}>
            Auction #{auctionId ? auctionId + 1 : 1}
          </h1>
  ...
  const fillPercentage =
    (auction.totalConverted / auction.tokenIds.length) * 100;
  ...
  const handleSoftBid = async () => {
    ...
    try {
      const desiredCount = Math.floor(
        (auction.tokenIds.length * softBidFraction) / 100,
      );
  ...
  const handleHardBid = async () => {
    ...
    try {
      const desiredCount = Math.floor(
        (auction.tokenIds.length * hardBidFraction) / 100,
      );
        ...
        >
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
              <div>
                <h1 className={title({ class: "gradient-metal" })}>
                  Auction #{auction.id + 1}
                </h1>
                <p className="text-gray-300 mt-2">
                  {auction.tokenIds.length} Domain NFTs • Seller:{" "}
                  <Link
                    isExternal
                    showAnchorIcon
                    color="success"
                ...
                <CardBody>
                  ...
                  <div className="mt-6">
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span>
                        {(parseFloat(auction.priceDecrement) * 30).toFixed(6)}{" "}
                        ETH per min
                      </span>
                    </div>
                    <Progress
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span>
                          {auction.totalConverted} / {auction.tokenIds.length}
                        </span>
                      </div>
                      <Progress
                    ...
                    <div className="grid md:grid-cols-2 gap-4 text-sm">
                      ...
                      <div className="flex justify-between">
                        <span>Remaining:</span>
                        <span className="font-semibold">
                          {auction.tokenIds.length - auction.totalConverted}{" "}
                          tokens
                        </span>
                      </div>
                    </div>
                          ...
                          >
                            ...
                            <div className="text-center">
                              <Chip
                                ...
                                onClick={() =>
                                  window.open(
                                    `${process.env.NEXT_PUBLIC_DOMA_BLOCK_EXPLORER}/token/${CONTRACT_ADDRESSES.OWNERSHIP_TOKEN}/instance/${domain.id}`,
                                    "_blank",
                                  )
                                }
                              >
                  ...
                  {!isConnected ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="text-xs text-gray-500 space-y-1">
                        <p>
                          • Soft Bid: Auto-converts when price drops to your
                          threshold
                        </p>
          ...
          <ModalContent>
            ...
            <ModalBody>
              <div className="space-y-4">
                <Alert
                  color="primary"
                  title={`Current price: ${parseFloat(auction.currentPrice).toFixed(8)} ETH per Domain`}
                />
